...
namespace grpcServer.Services
{
    public class FileTransportService : FileService.FileServiceBase
    {
        ...
        {
            //Stream'in yapılacağı dizini belirliyelim
            ...
            try
            {
                ...
                while (await requestStream.MoveNext())
                {
                    if (count++ == 0)
                    {
                        fileStream = new($"{path}/{requestStream.Current.Info.FileName}{requestStream.Current.Info.FileExtension}", FileMode.CreateNew);
